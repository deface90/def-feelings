...
func TestErrorDetailsMsg(t *testing.T) {
	callerFn := func() {
		...
		assert.Equal(t, "error details 123456 - error 500 - 500 (123) - 1.2.3.4 - https://example.com/test?k1=v1&k2=v2 [caused by backend/rest/httperrors_test.go:46 rest.TestErrorDetailsMsg]", msg)
	}
	callerFn()
}
...
func TestErrorDetailsMsgWithUser(t *testing.T) {
	callerFn := func() {
		...
		assert.Equal(t, "error details 123456 - error 500 - 500 (34567) - 127.0.0.1 - https://example."+
			"com/test?k1=v1&k2=v2 [caused by backend/rest/httperrors_test.go:59 rest.TestErrorDetailsMsgWithUser]", msg)
	}
	callerFn()
}
